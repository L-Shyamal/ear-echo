...
#define NRF_PDM_FREQ_6144K  (nrf_pdm_freq_t)(0x48000000UL)               ///< PDM_CLK= 6.144 MHz (32 MHz /  5) => Fs= 96000 Hz [Ratio80 => Fs= ?] 




PDMClass2::PDMClass2(int dinPin, int clkPin, int pwrPin) :
  _dinPin(dinPin),
  _clkPin(clkPin),
  _pwrPin(pwrPin),
  _onReceive(NULL),
...
*/
int PDMClass2::begin(int channels, int sampleRate,bool high)
{
  _channels = channels;
  ...
  switch (sampleRate) {
    ...
    case 96000:
      // Untested
      nrf_pdm_clock_set(NRF_PDM_FREQ_6144K);
      break;
...
PDMClass2 PDM2(PIN_PDM_DIN, PIN_PDM_CLK, PIN_PDM_PWR);
